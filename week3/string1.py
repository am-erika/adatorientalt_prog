...
def fix_start(s):
    first_letter = s[0]
    rep_letter = "*"
    remaining_letters = s.removeprefix(first_letter)
    modif_remaining_letters = remaining_letters.replace(first_letter, "*")
    return "".join([first_letter, modif_remaining_letters]) 
    #return first_letter + modif_remaining_letters - Solution 2 for concatenating
...
def mix_up(a, b):
    ...
    last_a = a[2:]
    last_b = b[2:]
    modif_a = "".join([firsttwo_b, last_a])
    modif_b = "".join([firsttwo_a, last_b])
    return " ".join([modif_a, modif_b])
